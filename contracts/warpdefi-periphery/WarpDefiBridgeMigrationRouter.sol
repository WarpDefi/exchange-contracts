...
import "../warpdefi-lib/libraries/TransferHelper.sol";
import "./interfaces/IBridgeToken.sol";
import "./libraries/Roles.sol";
import "./libraries/WarpDefiLibrary.sol";

contract WarpDefiBridgeMigrationRouter {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'WarpDefiBridgeMigrationRouter: EXPIRED');
        _;
    }

    // makes sure the admin is the one calling protected methods
    modifier onlyAdmin() {
        require(adminRole.has(msg.sender), 'WarpDefiBridgeMigrationRouter: Unauthorized');
        _;
    }
    ...
    function addAdmin(address account) external onlyAdmin {
        require(account != address(0), "WarpDefiBridgeMigrationRouter: Address 0 not allowed");
    ...
    function removeAdmin(address account) external onlyAdmin {
        require(msg.sender != account, "WarpDefiBridgeMigrationRouter: You can't demote yourself");
    ...
    function addMigrator(address tokenAddress, address migratorAddress) external onlyAdmin {
        require(tokenAddress != address(0), "WarpDefiBridgeMigrationRouter: tokenAddress 0 not supported");
        require(migratorAddress != address(0), "WarpDefiBridgeMigrationRouter: migratorAddress 0 not supported");
    ...
    function _allowToken(address tokenAddress, address spenderAddress) internal {
        IWarpDefiERC20(tokenAddress).approve(spenderAddress, type(uint).max);
    }
    ...
    function _addLiquidity(
        ...
        (uint112 reserve0, uint112 reserve1,) = IWarpDefiPair(pairToken).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = WarpDefiLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
            quote0 = WarpDefiLibrary.quote(amountIn1, reserve1, reserve0);
        }
        ...
        liquidityAmount = IWarpDefiPair(pairToken).mint(to);
    }
    ...
    function _rescueLiquidity(
        ...
        (amountTokenA, amountTokenB) = IWarpDefiPair(liquidityPair).burn(address(this));
    }
    ...
    function _arePairsCompatible(address pairA, address pairB) internal view {
        require(pairA != address(0), "WarpDefiBridgeMigrationRouter: liquidityPairFrom address 0");
        require(pairA != address(0), "WarpDefiBridgeMigrationRouter: liquidityPairTo address 0");
        require(pairA != pairB, "WarpDefiBridgeMigrationRouter: Cant convert to the same liquidity pairs");
        require(
            IWarpDefiPair(pairA).token0() == IWarpDefiPair(pairB).token0() ||
            IWarpDefiPair(pairA).token0() == IWarpDefiPair(pairB).token1() ||
            IWarpDefiPair(pairA).token1() == IWarpDefiPair(pairB).token0() ||
            IWarpDefiPair(pairA).token1() == IWarpDefiPair(pairB).token1(),
            "WarpDefiBridgeMigrationRouter: Pair does not have one token matching"
        );
    }
    ...
    function _migrateToken(
        ...
        require(
            ...
            "WarpDefiBridgeMigrationRouter: Didn't yield the correct amount"
        );
    }
    ...
    function migrateToken(
        ...
        require(
            ...
            "WarpDefiBridgeMigrationRouter: migrator not registered"
        );
    ...
    function migrateLiquidityWithPermit(
        ...
        IWarpDefiPair(liquidityPairFrom).permit(msg.sender, address(this), amount, deadline, v, r, s);
        _migrateLiquidity(
            liquidityPairFrom,
            liquidityPairTo,
            to,
    ...
    function _migrateLiquidity(
        ...
        address tokenToMigrate = IWarpDefiPair(liquidityPairFrom).token0();
        if (
            IWarpDefiPair(liquidityPairFrom).token0() == IWarpDefiPair(liquidityPairTo).token0() ||
            IWarpDefiPair(liquidityPairFrom).token0() == IWarpDefiPair(liquidityPairTo).token1()
        ) {
            tokenToMigrate = IWarpDefiPair(liquidityPairFrom).token1();
        }
        address newTokenAddress = bridgeMigrator[tokenToMigrate];
        require(
            newTokenAddress != address(0),
            "WarpDefiBridgeMigrationRouter: Migrator not registered for the pair"
        );
        require(
            newTokenAddress == IWarpDefiPair(liquidityPairTo).token0() ||
            newTokenAddress == IWarpDefiPair(liquidityPairTo).token1(),
            "WarpDefiBridgeMigrationRouter: Pair doesn't match the migration token"
        );

        (uint amountTokenA, uint amountTokenB) = _rescueLiquidity(liquidityPairFrom, amount);
        {
            uint amountToSwap = amountTokenA;
            if (tokenToMigrate != IWarpDefiPair(liquidityPairFrom).token0()) {
                amountToSwap = amountTokenB;
            }
            _migrateToken(tokenToMigrate, amountToSwap);
        }
        if (IWarpDefiPair(liquidityPairFrom).token0() != IWarpDefiPair(liquidityPairTo).token0() &&
            IWarpDefiPair(liquidityPairFrom).token1() != IWarpDefiPair(liquidityPairTo).token1()
        ) {
        ...
        (uint changeAmount0, uint changeAmount1, ) = _addLiquidity(
            liquidityPairTo,
            IWarpDefiPair(liquidityPairTo).token0(), IWarpDefiPair(liquidityPairTo).token1(),
            amountTokenA, amountTokenB, to
        );
        if (changeAmount0 > 0) {
            TransferHelper.safeTransfer(IWarpDefiPair(liquidityPairTo).token0(), to, changeAmount0);
        }
        if (changeAmount1 > 0) {
            TransferHelper.safeTransfer(IWarpDefiPair(liquidityPairTo).token1(), to, changeAmount1);
        }
    }
    ...
    function _calculateRescueLiquidity(address pairAddress, uint amount) internal view returns (uint amount0, uint amount1) {
        (uint112 reserves0, uint112 reserves1, ) = IWarpDefiPair(pairAddress).getReserves();
        uint totalSupply = IWarpDefiPair(pairAddress).totalSupply();
    ...
    function calculateChargeBack(
        ...
        require(liquidityPairFrom != address(0), "WarpDefiBridgeMigrationRouter: liquidityPairFrom address 0 not supported");
        require(liquidityPairTo != address(0), "WarpDefiBridgeMigrationRouter: liquidityPairTo address 0 not supported");
        ...
        (uint112 reserve0, uint112 reserve1,) = IWarpDefiPair(liquidityPairTo).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = WarpDefiLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
