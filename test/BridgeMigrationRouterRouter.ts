...
describe("BridgeMigrationRouter", async function() {
    ...
    type PNGPairsTokenSymbol = keyof typeof fixture.Pairs.WARP
    type PNGMigratedPairsTokenSymbol = keyof typeof fixture.Pairs.Migrated.WARP
    ...
    before(async () => {
        ...
        await (await getTokenContract(fixture.Tokens.WARP)).approve(router.address, ethers.constants.MaxUint256)
        ...
        // if there's no pairs in the migrators, create the pairs and add liquidity for WARP pairs
        for(let tokenSymbol of Object.keys(fixture.Pairs.Migrated.WARP)) {
            let tokenAddress = fixture.Migrators[tokenSymbol as MigratorTokenSymbol]
            if (!tokenAddress) continue
            if (fixture.Pairs.Migrated.WARP[tokenSymbol as PNGMigratedPairsTokenSymbol] !== "") {
                continue
            }
            const price = BigNumber.from("1000000000000000000")
            await router.connect(owner).addLiquidity(
                fixture.Tokens.WARP, tokenAddress,
                price, price.mul(2), price, price.mul(2),
                owner.address, getDeadline()
            )

            let bn0 = BigNumber.from(fixture.Tokens.WARP)
            ...
            fixture.Pairs.Migrated.WARP[tokenSymbol as PNGMigratedPairsTokenSymbol] = await factory.getPair(token0, token1)
        }
    })
    ...
    describe("Liquidity Migration", async function() {
        ...
        describe("WARP", async function() {
            for(let tokenSymbol of Object.keys(fixture.Migrators)) {
                let tokenAddress = fixture.Tokens[tokenSymbol as TokenSymbol]
                if (!((tokenSymbol as AVAXPairsTokenSymbol) in fixture.Pairs.WARP)) continue
                if (fixture.TokensWithoutFund.includes(tokenSymbol)) continue
                it(`Can migrate liquidity from WARP-${tokenSymbol}`, async function() {
                    let pairAddress = fixture.Pairs.WARP[tokenSymbol as PNGPairsTokenSymbol]
                    let toPairAddress = fixture.Pairs.Migrated.WARP[tokenSymbol as PNGPairsTokenSymbol]
                    let migrator = fixture.Migrators[tokenSymbol as MigratorTokenSymbol]
                    await migrationRouter.connect(owner).addMigrator(tokenAddress, migrator)
                    let liquidityAmount = await fundLiquidityToken(account, pairAddress, BigNumber.from(10).pow(20))
                    let fromPairContract = await getTokenContract(pairAddress)
                ...
                it(`Can compute accurately chargeback from WARP-${tokenSymbol}`, async function() {
                    let pairAddress = fixture.Pairs.AVAX[tokenSymbol as AVAXPairsTokenSymbol]
                    let toPairAddress = fixture.Pairs.Migrated.AVAX[tokenSymbol as AVAXMigratedPairsTokenSymbol]
                    let migrator = fixture.Migrators[tokenSymbol as MigratorTokenSymbol]
                    await migrationRouter.connect(owner).addMigrator(tokenAddress, migrator)
